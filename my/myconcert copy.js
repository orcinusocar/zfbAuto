...
const pageHandlers = {
    ...
    tabPage: {
        check: () => {
            //tab
            return id("com.sankuai.movie:id/db5").findOne(2000) !== null;
        },
        action: () => {
            console.log("进入首页");
            intoMainPage();
...
function intoMainPage(){
    let mainPageParent = id("com.sankuai.movie:id/db5").findOne(2000).parent();
    if (mainPageParent) {
...
function getConcertName() {
    ...
    while (retryCount < maxRetries) {
        let ConcertNameControls = boundsInside(365, 258, 1036, 500).clickable().findOne(2000);
        if (ConcertNameControls) {
            let text = ConcertNameControls.text();
            if (text && text.trim() !== "") {
...
function intoConcert() {
    ...
    while(!concertTab){
        sleep(3000);
        concertTab = id("com.sankuai.movie:id/ca3").text("演唱会").findOnce();
    }
    if (concertTab) {
        concertTab.click();
        sleep(5000);
